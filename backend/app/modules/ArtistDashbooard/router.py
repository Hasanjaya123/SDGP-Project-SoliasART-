...
async def get_full_artist_profile(current_user: str = Depends(get_current_user)):
    try:
        # Safely extract the ID whether current_user is an object or a string token
        ...
        sold_artworks = supabase.table("artwork").select("id", count="exact").eq("artist_id", artist_id).eq("status", "sold").execute()
        
        # FIXED: Removed 'sold_at', 'buyer_name' and changed 'likes_count' to 'likes'
        get_listed_artwork = supabase.table("artwork").select(
            "id, title, price, image_url, width_in, height_in, medium, status, view_count, likes, artists(display_name)"
        ).eq("artist_id", artist_id).execute()
       
        tot_views = 0
        for view in views.data:
            tot_views += int(view.get("view_count") or 0)
            
        tot_revenue = 0
        for rev in revenue.data:
            tot_revenue += float(rev.get("price") or 0.0)   
