...
@router.post("/trigger-ar/{artwork_id}")
async def trigger_ar_generation(artwork_id: str, db: Session = Depends(get_db)):
    # This does the exact same processing, but returns a simple JSON message 
    # instead of the binary file, bypassing Choreo's binary/CORS restrictions.
    ...
    try:
        ...
        # Return JSON instead of the GLB Blob
        return {"status": "success", "message": "Model generated successfully"}
        
    except Exception as e:
        if os.path.exists(cached_glb_path): os.remove(cached_glb_path)
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if os.path.exists(temp_img): os.remove(temp_img)