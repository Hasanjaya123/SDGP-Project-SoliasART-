...
def create_artwork(data: ArtworkCreate, image_url: str):
    """
    Saves artwork metadata to the 'artwork' table.
    """
    try:
        ...
        res = supabase.table('artwork').insert(artwork_dict).execute()
        return res.data[0]
    except Exception as e:
...
def get_artworks_by_artist(artist_id: str):
    ...
    """
    try:
        res = supabase.table('artwork').select('*').eq('artist_id', artist_id).execute()
        return res.data
    except Exception as e:
